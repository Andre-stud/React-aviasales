...
function MenuFilter() {
  ...
  const basicCheckboxesStatus = activeCheckbox[1].isActive && activeCheckbox[2].isActive
                                && activeCheckbox[3].isActive && activeCheckbox[4].isActive;
  ...
  const checkboxElements = checkboxData.map(({nameCheckbox, label}, index)=>(
      <li key={label}>
        <Checkbox className='toggle' checked={activeCheckbox[index].isActive} name={nameCheckbox} onChange={onChange}>{label} </Checkbox>
      </li>
    ));

  return (
    <div className="menu-filter">
        <p className='number-of-transfers'>
